...
class GenerateLlmTests(unittest.TestCase):
    def test_build_library_snapshot_limits_read_and_to_read(self):
        books_payload = {
            "stats": {"total_read": 150, "total_to_read": 700},
            "books": {
                "read": [
                    {
                        "title": f"Read {index}",
                        "author": "Author",
                        "my_rating": 4,
                        "my_review": f"Review {index}",
                        "shelves": ["history"],
                        "date_read": f"2026-01-{(index % 28) + 1:02d}",
                    }
                    for index in range(150)
                ],
                "currently_reading": [
                    {"title": "Current 1", "author": "Author A"},
                    {"title": "Current 2", "author": "Author B"},
                ],
                "to_read": [
                    {"title": f"Future {index}", "author": f"Author {index}"}
        ...
        self.assertEqual(len(snapshot["read"]), generate_llm.MAX_PROMPT_READ_BOOKS)
        self.assertEqual(len(snapshot["currently_reading"]), 2)
        self.assertEqual(len(snapshot["to_read"]), generate_llm.MAX_PROMPT_TO_READ_BOOKS)

    def test_build_library_snapshot_samples_to_read_books(self):
        books_payload = {
            "stats": {},
            "books": {
                "read": [],
        ...
        sampled_books = books_payload["books"]["to_read"][100:600]

        with patch.object(generate_llm.random, "sample", return_value=sampled_books) as sample_mock:
            snapshot = generate_llm.build_library_snapshot(books_payload)

        self.assertEqual(snapshot["to_read"], sampled_books)
        sample_mock.assert_called_once_with(
            books_payload["books"]["to_read"],
            generate_llm.MAX_PROMPT_TO_READ_BOOKS,
        )

    def test_build_library_snapshot_can_exclude_to_read(self):
        books_payload = sample_books_payload()

        snapshot = generate_llm.build_library_snapshot(books_payload, include_to_read=False)

        self.assertEqual(snapshot["to_read"], [])

    def test_build_library_snapshot_respects_custom_read_limit(self):
        books_payload = {
            "stats": {},
            "books": {
                "read": [
        ...
        snapshot = generate_llm.build_library_snapshot(books_payload, max_read_books=200)

        self.assertEqual(len(snapshot["read"]), 200)

    def test_build_library_snapshot_zero_read_limit_includes_all_reads(self):
        books_payload = {
            "stats": {},
            "books": {
                "read": [
        ...
        snapshot = generate_llm.build_library_snapshot(books_payload, max_read_books=0)

        self.assertEqual(len(snapshot["read"]), 250)
